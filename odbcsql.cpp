...
void createhandle(SQLHENV  hEnv, SQLHDBC   hDbc, SQLHSTMT   hStmt);
...
void createhandle(SQLHENV  hEnv, SQLHDBC   hDbc, SQLHSTMT   hStmt ,bool  source)
{
    ...
    while (_fgetts(wszInput, SQL_QUERY_SIZE - 1,stdin))
    {
        ...
        case SQL_SUCCESS:
        {
            ...
            if (sNumResults > 0)
            {
                DisplayResults(hStmt, sNumResults,source);
            }
            else
            {
                SQLLEN cRowCount;
...
int __cdecl wmain()
{
   ...
   // WCHAR* pwszConnStr2 = NULL;
  //  WCHAR       wszInput[SQL_QUERY_SIZE];
   // WCHAR      wszInput2[SQL_QUERY_SIZE];

     createhandle(&hEnv, &hDbc, &hStmt,true);
     createhandle(&hEnv2, &hDbc2,&hStmt2, false);
}
/************************************************************************
...
Exit:
    ...
    while (pFirstBinding)
    {
      ...
      //  *resultdata[index] = *pFirstBinding->wszBuffer;
            resultdata[index] = (WCHAR*)malloc(100 * sizeof(WCHAR));
            swprintf(resultdata[index], pFirstBinding->wszBuffer);
        index++;
        }
